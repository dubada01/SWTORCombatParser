...
namespace SWTORCombatParser.Model.Plotting
{
    public class PlotMaker
    {
        ...
        {
            return logs.Where(l =>
                    l.Target == sourcePlayer && l.Effect.EffectType != EffectType.AbsorbShield
                )
                .Select(l => ((double)l.TargetInfo.CurrentHP / (double)l.TargetInfo.MaxHP))
                .ToArray();
        }

        internal static double[] GetPlotHPXVals(
            List<ParsedLogEntry> totalLogsDuringCombat,
            DateTime startPoint,
            Entity sourcePlayer
        )
        {
            var startTime = startPoint;
            var logsToUse = totalLogsDuringCombat.Where(l =>
                l.Target == sourcePlayer && l.Effect.EffectType != EffectType.AbsorbShield
            );
            return logsToUse.Select(l => (l.TimeStamp - startTime).TotalSeconds).ToArray();
        }

        internal static double[] GetPlotXVals(
            IEnumerable<ParsedLogEntry> totalLogsDuringCombat,
            DateTime startPoint
        )
        {
            var startTime = startPoint;
            return totalLogsDuringCombat
                .Select(l => (l.TimeStamp - startTime).TotalSeconds)
                .ToArray();
        }

        internal static double[] GetPlotYVals(
            IEnumerable<ParsedLogEntry> totalLogsDuringCombat,
            bool checkEffective
        )
        {
        ...
        {
            var timeStampsSpread = Enumerable
                .Range((int)timeStamps.First(), (int)(timeStamps.Last() - timeStamps.First()))
                .ToList();
            return timeStampsSpread.Select(d => (double)d).ToArray();
        }

        internal static double[] GetPlotYValRates(
            double[] yValues,
            double[] timeStamps,
            double averageWindowDuration = 10
        )
        {
            ...
            for (int i = 0; i < timeStampsSpread.Count; i++)
            {
                ...
                {
                    movingaverage[i] = movingAverageCalc.ComputeAverage(
                        spread,
                        timeStampsSpread[i]
                    );
                }
        ...
        internal static List<(string, string)> GetAnnotationString(
            List<ParsedLogEntry> data,
            bool isIncoming,
            bool isShield = false
        )
        {
        ...
        private static (string, string) GetAnnotationForAbilitiy(
            ParsedLogEntry log,
            bool isIncoming,
            bool isShield
        )
        {
            ...
            if (
                ...
                && log.Value.Modifier != null
            )
            {
                stringToShow +=
                    "\nMitigation: "
                    + log.Value.Modifier.EventType.ToString()
                    + "-"
                    + log.Value.Modifier.EffectiveDblValue;
            }
    ...
    public class MovingAverage
    {
        ...
        public double ComputeAverage(double newSample, double timeStamp)
        {
            ...
            while (
                TimeSpan.FromSeconds(samples.Last().Item2 - samples.First().Item2) > windowDuration
            )
            {
            ...
            Average =
                sampleAccumulator
                / (samples.Count == 1 ? 1 : (samples.Last().Item2 - samples.First().Item2));
