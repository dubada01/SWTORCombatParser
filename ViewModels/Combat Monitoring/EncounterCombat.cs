        ...
        public void AddCombat(Combat combat, bool isReplacingOngoing)
        {
            lock (combatAddLock)
            {
                ...
                var pastCombatDisplay = new PastCombat()
                {
                    ...
                    CombatLabel = combat.IsCombatWithBoss ? combat.EncounterBossDifficultyParts.Item1 : combat.IsPvPCombat ? GetPVPCombatText(combat) : string.Join(',', combat.Targets.Values.Select(t => t.Name).Distinct()),
                };
